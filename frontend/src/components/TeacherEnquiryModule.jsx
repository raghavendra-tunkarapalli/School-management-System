...
import { BookOpen, Search, RefreshCw, Users, UserCheck, MessageSquare, Send, CheckCircle2, Edit2, Clock } from 'lucide-react';

const GATEWAY_URL = 'http://localhost:8099/api/teacher/enquiries';
const DIRECT_URL = 'http://localhost:8087/api/teacher/enquiries';

export default function TeacherEnquiryModule() {
  ...
  const fetchEnquiries = async () => {
    ...
    try {
      ...
      console.warn('Failed to fetch teacher enquiries:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSendResponse = async (id, type) => {
    ...
    const primaryEndpoint = type === 'PARENT'
      ? `http://localhost:8099/api/teacher/enquiries/parent/${id}/response`
      : `http://localhost:8099/api/teacher/enquiries/student/${id}/response`;
  ...
  return (
    <div className="module-card">
      <div className="module-header">
        <div className="module-icon-badge teacher">
          <BookOpen size={22} color="#10b981" />
        </div>
        <div style={{ flex: 1 }}>
          <h2 className="module-title">Teacher Enquiry Management Module</h2>
          <p className="module-subtitle">
            Powered by <code>teacher-enquiry-service</code> (View and respond to student & parent inquiries, responses saved to MySQL)
          </p>
        </div>
      ...
      <div className="stats-grid-2" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem', marginBottom: '1.25rem' }}>
        <div className="stat-card" style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.25)', borderRadius: '12px', padding: '1rem' }}>
          <div style={{ fontSize: '0.8rem', color: '#34d399', textTransform: 'uppercase', fontWeight: '700' }}>Student Enquiries</div>
          <div style={{ fontSize: '1.75rem', fontWeight: '800', color: '#ffffff', marginTop: '0.2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            {studentPendingResponseCount > 0 ? (
              <span style={{ fontSize: '0.75rem', background: 'rgba(16, 185, 129, 0.2)', color: '#6ee7b7', border: '1px solid rgba(16, 185, 129, 0.4)', padding: '0.2rem 0.6rem', borderRadius: '12px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button
            className={`portal-tab-btn ${activeSubTab === 'STUDENT' ? 'active-teacher' : ''}`}
          ...
          <button
            className={`portal-tab-btn ${activeSubTab === 'PARENT' ? 'active-teacher' : ''}`}
              ...
              <tbody>
                {filteredStudents.length > 0 ? (
                  filteredStudents.map((enq) => {
                    ...
                    return (
                      <tr key={enq.id || enq.enquireId} style={{ background: !hasResponse ? 'rgba(16, 185, 129, 0.06)' : 'transparent' }}>
                        <td>
                        ...
                        <td>
                          {hasResponse ? (
                            ...
                            <span className="role-pill staff" style={{ background: 'rgba(16, 185, 129, 0.2)', color: '#34d399', borderColor: 'rgba(16, 185, 129, 0.5)', fontSize: '0.75rem', padding: '0.2rem 0.6rem', fontWeight: '700' }}>
                              • Pending Response
                            </span>
                          )}
                        </td>
                          ...
                          {hasResponse && !isEditing ? (
                            ...
                            <div style={{ display: 'flex', gap: '0.4rem', width: '100%', alignItems: 'center' }}>
                              ...
                              <button
                                ...
                                className="btn-submit teacher-btn"
                                style={{
                                  width: 'auto',
                                  minWidth: 'auto',
