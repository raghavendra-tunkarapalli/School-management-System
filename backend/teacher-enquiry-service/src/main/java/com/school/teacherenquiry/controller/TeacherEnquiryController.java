package com.school.teacherenquiry.controller;

import com.school.teacherenquiry.dto.TeacherEnquiryDTO;
...
@RestController
@RequestMapping("/api/teacher/enquiries")
@CrossOrigin(origins = "*")
public class TeacherEnquiryController {

    private final RestTemplate restTemplate;

    public TeacherEnquiryController() {
        this.restTemplate = new RestTemplate();
    }

    @GetMapping
    public ResponseEntity<List<TeacherEnquiryDTO>> getAllEnquiries() {
        List<TeacherEnquiryDTO> aggregatedList = new ArrayList<>();
        ...
        try {
            ...
            if (studentEnquiries != null) {
                for (Map<String, Object> item : studentEnquiries) {
                    TeacherEnquiryDTO dto = new TeacherEnquiryDTO();
    ...
    public ResponseEntity<?> respondToStudentEnquiry(@PathVariable Long id, @RequestBody Map<String, String> body) {
        try {
            ...
            return ResponseEntity.ok(Map.of("message", "Teacher response sent to student enquiry successfully and marked as RESPONDED"));
        } catch (Exception e) {
    ...
    public ResponseEntity<?> respondToParentEnquiry(@PathVariable Long id, @RequestBody Map<String, String> body) {
        try {
            ...
            return ResponseEntity.ok(Map.of("message", "Teacher response sent to parent enquiry successfully and marked as RESPONDED"));
        } catch (Exception e) {
            return ResponseEntity.internalServerError().body(Map.of("error", "Failed to send response: " + e.getMessage()));
        }
    }
